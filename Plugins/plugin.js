...
module.exports = {
  ...
  start: async (Atlas, m, { text, args, pushName, prefix, inputCMD, isCreator, isintegrated, doReact, botImage1 }) => {
    switch (inputCMD) {
      case "install":
       ...
       try{
           const res = await got(url);
             ...
             await doReact("❌");
               m.reply(`An error occurred while fetching plugin code from the URL.`);
               console.error(error);
               return;
         }
        ...
        if (statusCode == 200) {
           try {
            ...
            var filePath = path.join(folderName, fileName);
             ...
             await doReact("❌");
             m.reply(
              `An error occurred while installing plugin`
            );
            console.error(error);
            return;
         }
          ...
          try{
              ...
              await m.reply(`*${fileName}* Installed Successfully !`);
                ...
                await doReact("❌");
              m.reply(
                 `An error Occurd while installing plugin`
                );
               console.error(error);
           }
        }
      ...
      case "plugins":
        ...
        try{
            ...
            if (!plugins.length) {
             ...
             await doReact("❌");
                m.reply(`An error occurred while fetching plugins.`);
             console.error(error);
        }
        break;
...
⚜️ To install a plugin type *install* _plugin-url_ !\n\nExample: *${prefix}install* https://gist.githubusercontent.com/FantoX001/xyz...\n\n⚜️ To uninstall a plugin type *uninstall* _plugin-name_ !\n\nExample: *${prefix}uninstall* audioEdit.js\n`;
       try{
            ...
            await doReact("❌");
              m.reply(
                 `An error Occurd while fetching plugin list.`
              );
           console.error(error);
       }
        break;
